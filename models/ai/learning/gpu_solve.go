...
func GPUSolveWithStats(g game.Game, player game.Player, depth int, perfStats *stats.PerformanceStats) (game.Position, bool) {
	...
	// Try to find a cached result for this board first
	...
	if found && cachedMove.Row >= 0 && cachedMove.Row < 8 && cachedMove.Col >= 0 && cachedMove.Col < 8 {
		...
		if game.IsValidMove(g.Board, player.Color, cachedMove) {
			...
			cacheHitTime := time.Since(totalStart)
			perfStats.RecordOperation("cache_hit", cacheHitTime)

			// Record source of cache hit
	...
	boardCache.Store(g.Board, player.Color, depth, int(result), foundMove, cache.SourceGPU)
